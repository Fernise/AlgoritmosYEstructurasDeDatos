...
{ 
  ...
  if ((i <= matrix_.get_m()) && (j <= matrix_.get_n()) && (i >= 1) && (j >= 1) && (matrix_.at(i, j) != 1) && (!visited_.at(i, j))) {
    return true;
  }
  return false;
}
...
// Modificación
bool maze_t::gate() {
  for (int i = 0; i <= matrix_.get_n(); ++i) {
    std::cout << matrix_.get_row(1) << std::endl;
  }
  return false;
}
